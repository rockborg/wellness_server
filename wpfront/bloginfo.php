...
class cblog extends cTable {
	...
	function __construct() {
		...
		// blogid
		$this->blogid = new cField('blog', 'blog', 'x_blogid', 'blogid', '`blogid`', '`blogid`', 3, -1, FALSE, '`blogid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->blogid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['blogid'] = &$this->blogid;

		// blogname
		$this->blogname = new cField('blog', 'blog', 'x_blogname', 'blogname', '`blogname`', '`blogname`', 200, -1, FALSE, '`blogname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['blogname'] = &$this->blogname;

		// blogdesc
		$this->blogdesc = new cField('blog', 'blog', 'x_blogdesc', 'blogdesc', '`blogdesc`', '`blogdesc`', 201, -1, FALSE, '`blogdesc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['blogdesc'] = &$this->blogdesc;

		// blogimage
		$this->blogimage = new cField('blog', 'blog', 'x_blogimage', 'blogimage', '`blogimage`', '`blogimage`', 200, -1, TRUE, '`blogimage`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->blogimage->UploadAllowedFileExt = "jpg,png,jpeg";
		$this->fields['blogimage'] = &$this->blogimage;

		// bloglink
		$this->bloglink = new cField('blog', 'blog', 'x_bloglink', 'bloglink', '`bloglink`', '`bloglink`', 200, -1, FALSE, '`bloglink`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['bloglink'] = &$this->bloglink;

		// createddate
		$this->createddate = new cField('blog', 'blog', 'x_createddate', 'createddate', '`createddate`', 'DATE_FORMAT(`createddate`, \'%d/%m/%Y %H:%i:%s\')', 135, 11, FALSE, '`createddate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->createddate->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['createddate'] = &$this->createddate;

		// categoryid
		$this->categoryid = new cField('blog', 'blog', 'x_categoryid', 'categoryid', '`categoryid`', '`categoryid`', 3, -1, FALSE, '`categoryid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('blogid', $rs))
				ew_AddFilter($where, ew_QuotedName('blogid') . '=' . ew_QuotedValue($rs['blogid'], $this->blogid->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->blogid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@blogid@", ew_AdjustSql($this->blogid->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("blogview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("blogview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "blogadd.php?" . $this->UrlParm($parm);
		else
			return "blogadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("blogedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("blogadd.php", $this->UrlParm($parm));
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->blogid->CurrentValue)) {
			$sUrl .= "blogid=" . urlencode($this->blogid->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->blogid->Exportable) $Doc->ExportCaption($this->blogid);
					if ($this->blogname->Exportable) $Doc->ExportCaption($this->blogname);
					if ($this->blogdesc->Exportable) $Doc->ExportCaption($this->blogdesc);
					if ($this->blogimage->Exportable) $Doc->ExportCaption($this->blogimage);
					if ($this->bloglink->Exportable) $Doc->ExportCaption($this->bloglink);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->blogid->Exportable) $Doc->ExportField($this->blogid);
						if ($this->blogname->Exportable) $Doc->ExportField($this->blogname);
						if ($this->blogdesc->Exportable) $Doc->ExportField($this->blogdesc);
						if ($this->blogimage->Exportable) $Doc->ExportField($this->blogimage);
						if ($this->bloglink->Exportable) $Doc->ExportField($this->bloglink);
