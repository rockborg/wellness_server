...
$website_webimage_blobview = NULL; // Initialize page object first

class cwebsite_webimage_blobview extends cwebsite {
	...
	// Page object name
	var $PageObjName = 'website_webimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('website', 'x_webimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('webimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('webimage')));
				if (trim(strval($this->Recordset->fields('webimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('webimage') . "\"");
				}
...
// Create page object
if (!isset($website_webimage_blobview)) $website_webimage_blobview = new cwebsite_webimage_blobview();

// Page init
$website_webimage_blobview->Page_Init();

// Page main
$website_webimage_blobview->Page_Main();
?>
<?php
$website_webimage_blobview->Page_Terminate();
?>
