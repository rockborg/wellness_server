...
$event_add = NULL; // Initialize page object first

class cevent_add extends cevent {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["eventid"] != "") {
				...
				$this->setKey("eventid", $this->eventid->CurrentValue); // Set up key
			} else {
				$this->setKey("eventid", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "eventview.php")
	...
	function LoadDefaultValues() {
		$this->_eventname->CurrentValue = NULL;
		$this->_eventname->OldValue = $this->_eventname->CurrentValue;
		$this->eventdesc->CurrentValue = NULL;
		$this->eventdesc->OldValue = $this->eventdesc->CurrentValue;
		$this->eventimage->Upload->DbValue = NULL;
		$this->eventimage->OldValue = $this->eventimage->Upload->DbValue;
		$this->eventimage->CurrentValue = NULL; // Clear file related field
		$this->eventlink->CurrentValue = NULL;
		$this->eventlink->OldValue = $this->eventlink->CurrentValue;
		$this->eventdate->CurrentValue = NULL;
		$this->eventdate->OldValue = $this->eventdate->CurrentValue;
		$this->createddate->CurrentValue = "0000-00-00 00:00:00";
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->eventimage);
			...
			$this->eventdate->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->eventdate->CurrentValue, 11));
	...
	function AddRow($rsold = NULL) {
		...
		// eventname
		$this->_eventname->SetDbValueDef($rsnew, $this->_eventname->CurrentValue, "", FALSE);
		...
		if (!$this->eventimage->Upload->KeepFile) {
			$this->eventimage->Upload->DbValue = ""; // No need to delete old file
		...
		// eventlink
		$this->eventlink->SetDbValueDef($rsnew, $this->eventlink->CurrentValue, "", FALSE);

		// eventdate
		$this->eventdate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->eventdate->CurrentValue, 11), NULL, FALSE);
		...
		if ($AddRow) {
			$this->eventid->setDbValue($conn->Insert_ID());
			$rsnew['eventid'] = $this->eventid->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($event_add)) $event_add = new cevent_add();
...
$event_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var event_add = new ew_Page("event_add");
event_add.PageID = "add"; // Page ID
var EW_PAGE_ID = event_add.PageID; // For backward compatibility

// Form object
var feventadd = new ew_Form("feventadd");

// Validate form
feventadd.Validate = function() {
...
feventadd.ValidateRequired = true;
<?php } else { ?>
feventadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
feventadd.Lists["x_categoryid"] = {"LinkField":"x_categoryid","Ajax":null,"AutoFill":false,"DisplayFields":["x_categoryname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $event_add->ShowPageHeader(); ?>
<?php
$event_add->ShowMessage();
?>
<form name="feventadd" id="feventadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($event_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $event_add->Token ?>">
<?php } ?>
...
<?php if ($event->eventdate->Visible) { // eventdate ?>
	<div id="r_eventdate" class="form-group">
		<label id="elh_event_eventdate" class="col-sm-2 control-label ewLabel"><?php echo $event->eventdate->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $event->eventdate->CellAttributes() ?>>
<span id="el_event_eventdate">
<input type="text" data-field="x_eventdate" name="x_eventdate" id="x_eventdate" placeholder="<?php echo ew_HtmlEncode($event->eventdate->PlaceHolder) ?>" value="<?php echo $event->eventdate->EditValue ?>"<?php echo $event->eventdate->EditAttributes() ?>>
</span>
<?php echo $event->eventdate->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
feventadd.Lists["x_categoryid"].Options = <?php echo (is_array($event->categoryid->EditValue)) ? ew_ArrayToJson($event->categoryid->EditValue, 1) : "[]" ?>;
</script>
</span>
