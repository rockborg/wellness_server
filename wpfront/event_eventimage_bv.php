...
$event_eventimage_blobview = NULL; // Initialize page object first

class cevent_eventimage_blobview extends cevent {
	...
	// Page object name
	var $PageObjName = 'event_eventimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('event', 'x_eventimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('eventimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('eventimage')));
				if (trim(strval($this->Recordset->fields('eventimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('eventimage') . "\"");
				}
...
// Create page object
if (!isset($event_eventimage_blobview)) $event_eventimage_blobview = new cevent_eventimage_blobview();

// Page init
$event_eventimage_blobview->Page_Init();

// Page main
$event_eventimage_blobview->Page_Main();
?>
<?php
$event_eventimage_blobview->Page_Terminate();
?>
