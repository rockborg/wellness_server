...
$app_add = NULL; // Initialize page object first

class capp_add extends capp {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["appid"] != "") {
				...
				$this->setKey("appid", $this->appid->CurrentValue); // Set up key
			} else {
				$this->setKey("appid", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "appview.php")
	...
	function LoadDefaultValues() {
		$this->appname->CurrentValue = NULL;
		$this->appname->OldValue = $this->appname->CurrentValue;
		$this->appdesc->CurrentValue = NULL;
		$this->appdesc->OldValue = $this->appdesc->CurrentValue;
		$this->appimage->Upload->DbValue = NULL;
		$this->appimage->OldValue = $this->appimage->Upload->DbValue;
		$this->appimage->CurrentValue = NULL; // Clear file related field
		$this->applink->CurrentValue = NULL;
		$this->applink->OldValue = $this->applink->CurrentValue;
		$this->appplatform->CurrentValue = NULL;
		$this->appplatform->OldValue = $this->appplatform->CurrentValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->appimage);
	...
	function AddRow($rsold = NULL) {
		...
		// appname
		$this->appname->SetDbValueDef($rsnew, $this->appname->CurrentValue, "", FALSE);

		// appdesc
		$this->appdesc->SetDbValueDef($rsnew, $this->appdesc->CurrentValue, "", FALSE);

		// appimage
		if (!$this->appimage->Upload->KeepFile) {
			$this->appimage->Upload->DbValue = ""; // No need to delete old file
		...
		// applink
		$this->applink->SetDbValueDef($rsnew, $this->applink->CurrentValue, "", FALSE);

		// appplatform
		$this->appplatform->SetDbValueDef($rsnew, $this->appplatform->CurrentValue, "", FALSE);
		...
		if ($AddRow) {
			$this->appid->setDbValue($conn->Insert_ID());
			$rsnew['appid'] = $this->appid->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($app_add)) $app_add = new capp_add();
...
// Page object
var app_add = new ew_Page("app_add");
app_add.PageID = "add"; // Page ID
var EW_PAGE_ID = app_add.PageID; // For backward compatibility

// Form object
var fappadd = new ew_Form("fappadd");

// Validate form
fappadd.Validate = function() {
...
fappadd.ValidateRequired = true;
<?php } else { ?>
fappadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fappadd.Lists["x_appplatform"] = {"LinkField":"x_plaformname","Ajax":null,"AutoFill":false,"DisplayFields":["x_plaformname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fappadd.Lists["x_categoryid"] = {"LinkField":"x_categoryid","Ajax":null,"AutoFill":false,"DisplayFields":["x_categoryname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $app_add->ShowPageHeader(); ?>
<?php
$app_add->ShowMessage();
?>
<form name="fappadd" id="fappadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($app_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $app_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fappadd.Lists["x_appplatform"].Options = <?php echo (is_array($app->appplatform->EditValue)) ? ew_ArrayToJson($app->appplatform->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fappadd.Lists["x_categoryid"].Options = <?php echo (is_array($app->categoryid->EditValue)) ? ew_ArrayToJson($app->categoryid->EditValue, 1) : "[]" ?>;
</script>
</span>
