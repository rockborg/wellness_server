...
class ccategory extends cTable {
	...
	function __construct() {
		...
		// categoryid
		$this->categoryid = new cField('category', 'category', 'x_categoryid', 'categoryid', '`categoryid`', '`categoryid`', 3, -1, FALSE, '`categoryid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->categoryid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['categoryid'] = &$this->categoryid;

		// categoryname
		$this->categoryname = new cField('category', 'category', 'x_categoryname', 'categoryname', '`categoryname`', '`categoryname`', 200, -1, FALSE, '`categoryname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['categoryname'] = &$this->categoryname;

		// categorydesc
		$this->categorydesc = new cField('category', 'category', 'x_categorydesc', 'categorydesc', '`categorydesc`', '`categorydesc`', 200, -1, FALSE, '`categorydesc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['categorydesc'] = &$this->categorydesc;

		// categoryimage
		$this->categoryimage = new cField('category', 'category', 'x_categoryimage', 'categoryimage', '`categoryimage`', '`categoryimage`', 200, -1, TRUE, '`categoryimage`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->categoryimage->UploadAllowedFileExt = "jpg,png,jpeg";
		$this->fields['categoryimage'] = &$this->categoryimage;

		// createddate
		$this->createddate = new cField('category', 'category', 'x_createddate', 'createddate', '`createddate`', 'DATE_FORMAT(`createddate`, \'%d/%m/%Y %H:%i:%s\')', 135, 11, FALSE, '`createddate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->createddate->UploadAllowedFileExt = ".jpg,.png";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('categoryid', $rs))
				ew_AddFilter($where, ew_QuotedName('categoryid') . '=' . ew_QuotedValue($rs['categoryid'], $this->categoryid->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@categoryid@", ew_AdjustSql($this->categoryid->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["categoryid"]; // categoryid
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->categorydesc->Exportable) $Doc->ExportCaption($this->categorydesc);
					if ($this->categoryimage->Exportable) $Doc->ExportCaption($this->categoryimage);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->categorydesc->Exportable) $Doc->ExportField($this->categorydesc);
						if ($this->categoryimage->Exportable) $Doc->ExportField($this->categoryimage);
