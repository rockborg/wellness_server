...
class cevent extends cTable {
	...
	function __construct() {
		...
		// eventid
		$this->eventid = new cField('event', 'event', 'x_eventid', 'eventid', '`eventid`', '`eventid`', 3, -1, FALSE, '`eventid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->eventid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['eventid'] = &$this->eventid;

		// eventname
		$this->_eventname = new cField('event', 'event', 'x__eventname', 'eventname', '`eventname`', '`eventname`', 200, -1, FALSE, '`eventname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['eventname'] = &$this->_eventname;

		// eventdesc
		$this->eventdesc = new cField('event', 'event', 'x_eventdesc', 'eventdesc', '`eventdesc`', '`eventdesc`', 200, -1, FALSE, '`eventdesc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['eventdesc'] = &$this->eventdesc;

		// eventimage
		$this->eventimage = new cField('event', 'event', 'x_eventimage', 'eventimage', '`eventimage`', '`eventimage`', 200, -1, TRUE, '`eventimage`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->eventimage->UploadAllowedFileExt = "jpg,png,jpeg";
		$this->fields['eventimage'] = &$this->eventimage;

		// eventlink
		$this->eventlink = new cField('event', 'event', 'x_eventlink', 'eventlink', '`eventlink`', '`eventlink`', 200, -1, FALSE, '`eventlink`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['eventlink'] = &$this->eventlink;

		// eventdate
		$this->eventdate = new cField('event', 'event', 'x_eventdate', 'eventdate', '`eventdate`', 'DATE_FORMAT(`eventdate`, \'%d/%m/%Y %H:%i:%s\')', 135, 11, FALSE, '`eventdate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->eventdate->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['eventdate'] = &$this->eventdate;

		// createddate
		$this->createddate = new cField('event', 'event', 'x_createddate', 'createddate', '`createddate`', 'DATE_FORMAT(`createddate`, \'%d/%m/%Y %H:%i:%s\')', 135, 11, FALSE, '`createddate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->createddate->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['createddate'] = &$this->createddate;

		// categoryid
		$this->categoryid = new cField('event', 'event', 'x_categoryid', 'categoryid', '`categoryid`', '`categoryid`', 3, -1, FALSE, '`categoryid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`event`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('eventid', $rs))
				ew_AddFilter($where, ew_QuotedName('eventid') . '=' . ew_QuotedValue($rs['eventid'], $this->eventid->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->eventid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@eventid@", ew_AdjustSql($this->eventid->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("eventview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("eventview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "eventadd.php?" . $this->UrlParm($parm);
		else
			return "eventadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("eventedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("eventadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("eventdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->eventid->CurrentValue)) {
			$sUrl .= "eventid=" . urlencode($this->eventid->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->eventid->Exportable) $Doc->ExportCaption($this->eventid);
					if ($this->_eventname->Exportable) $Doc->ExportCaption($this->_eventname);
					if ($this->eventdesc->Exportable) $Doc->ExportCaption($this->eventdesc);
					if ($this->eventimage->Exportable) $Doc->ExportCaption($this->eventimage);
					if ($this->eventlink->Exportable) $Doc->ExportCaption($this->eventlink);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->eventid->Exportable) $Doc->ExportField($this->eventid);
						if ($this->_eventname->Exportable) $Doc->ExportField($this->_eventname);
						if ($this->eventdesc->Exportable) $Doc->ExportField($this->eventdesc);
						if ($this->eventimage->Exportable) $Doc->ExportField($this->eventimage);
						if ($this->eventlink->Exportable) $Doc->ExportField($this->eventlink);
