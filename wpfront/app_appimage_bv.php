...
$app_appimage_blobview = NULL; // Initialize page object first

class capp_appimage_blobview extends capp {
	...
	// Page object name
	var $PageObjName = 'app_appimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('app', 'x_appimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('appimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('appimage')));
				if (trim(strval($this->Recordset->fields('appimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('appimage') . "\"");
				}
...
// Create page object
if (!isset($app_appimage_blobview)) $app_appimage_blobview = new capp_appimage_blobview();

// Page init
$app_appimage_blobview->Page_Init();

// Page main
$app_appimage_blobview->Page_Main();
?>
<?php
$app_appimage_blobview->Page_Terminate();
?>
