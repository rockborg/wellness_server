...
$category_categoryimage_blobview = NULL; // Initialize page object first

class ccategory_categoryimage_blobview extends ccategory {
	...
	// Page object name
	var $PageObjName = 'category_categoryimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('category', 'x_categoryimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('categoryimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('categoryimage')));
				if (trim(strval($this->Recordset->fields('categoryimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('categoryimage') . "\"");
				}
...
// Create page object
if (!isset($category_categoryimage_blobview)) $category_categoryimage_blobview = new ccategory_categoryimage_blobview();

// Page init
$category_categoryimage_blobview->Page_Init();

// Page main
$category_categoryimage_blobview->Page_Main();
?>
<?php
$category_categoryimage_blobview->Page_Terminate();
?>
