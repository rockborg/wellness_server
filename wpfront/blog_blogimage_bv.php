...
$blog_blogimage_blobview = NULL; // Initialize page object first

class cblog_blogimage_blobview extends cblog {
	...
	// Page object name
	var $PageObjName = 'blog_blogimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('blog', 'x_blogimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('blogimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('blogimage')));
				if (trim(strval($this->Recordset->fields('blogimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('blogimage') . "\"");
				}
...
// Create page object
if (!isset($blog_blogimage_blobview)) $blog_blogimage_blobview = new cblog_blogimage_blobview();

// Page init
$blog_blogimage_blobview->Page_Init();

// Page main
$blog_blogimage_blobview->Page_Main();
?>
<?php
$blog_blogimage_blobview->Page_Terminate();
?>
