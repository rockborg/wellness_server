 ...
 * @version 0.96 $Id: index.php 1050 2012-11-15 01:04:38Z marciuz $
...
<html>
	...
	*/
	...
	FROM 0.96beta3 PEAR is not required

	$class_alert['pear']=(@include_once("PEAR.php")) ? "install-ok" : "install-alert";
	echo "<p class=\"".$class_alert['pear']."\">"._("PEAR includes:")."<br />\n";
	echo "<small>"._("PEAR is used in creating statistical graphs")."</small></p>\n";

	$class_alert['pear_img_graph_class']=(@include_once("Image/Graph.php")) ? "install-ok" : "install-alert";
	echo "<p class=\"".$class_alert['pear_img_graph_class']."\">"._("PEAR Image/Graph module include:")."<br />\n";
	echo "<small>"._("PEAR module used for creating graphs")."</small></p>\n";


	$class_alert['pear_img_canvas_class']=(@include_once("Image/Canvas.php")) ? "install-ok" : "install-alert";
	echo "<p class=\"".$class_alert['pear_img_canvas_class']."\">"._("PEAR Image/Canvas module include:")."<br />\n";
	echo "<small>"._("PEAR module used to create statisctical graphs (dependency Image / Graph.php)")."</small></p>\n";


	$class_alert['pear_img_color_class']=(@include_once("Image/Color.php")) ? "install-ok" : "install-alert";
	echo "<p class=\"".$class_alert['pear_img_color_class']."\">"._("PEAR Image/Color module include:")."<br />\n";
	echo "<small>"._("PEAR module used to create statisctical graphs (dependency Image / Graph.php)")."</small></p>\n";
...
function crea_db_rules(){
	...
	if(!$q_test){
		...
		header("Location: ".$_SERVER['PHP_SELF']."?p=panic&errty=no_create_table");
		exit;
	}
	else{
	...
	if($db1['dbtype']=='mysql'){
		...
		$q0=@$vmsql->query($sql_creadb) or die("Impossibile creare il database {$db1['frontend']}");
	...
	for($i=0;$i<count($SQL_DEFINITION);$i++){
		...
		if(!$q_creatabelle){
			$xerr=$vmsql->error($SQL_DEFINITION[$i],_('Error in sql execution'));
