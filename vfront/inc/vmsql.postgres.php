...
* @version 0.96 $Id: vmsql.postgres.php 1144 2015-04-24 14:51:22Z marciuz $
...
class postgres_vmsql {
	...
	*/
	public function connect($array_db,$charset=''){
		...
		$this->link_db = @pg_connect("host={$array_db['host']} port={$array_db['port']} dbname={$array_db['postgres_dbname']} user={$array_db['user']} password={$array_db['passw']}");
		...
		if(is_resource($this->link_db) && $charset!=''){
	...
	public function query($sql,$transazione=false){
		...
		if(is_resource($this->link_db) && $this->connected){
			...
			$resource = pg_get_result($this->link_db);
            ...
            $this->error($sql,"Link al DB non disponibile");
        }
