...
/*
	...
	`categoryid` int(11) NOT NULL, INDEX(`categoryid`), PRIMARY KEY  (`userid`)) ENGINE=MyISAM;
*/
...
class user extends POG_Base
{
	...
	*/
	...
	function user($firstname='', $lastname='', $email='', $password='', $regdate='', $lastlogindate='')
	{
	...
	*/
	function Save()
	{
		...
		if ($rows > 0)
		{
			...
			`categoryid`='".$this->categoryId."' where `userid`='".$this->userId."'";
		}
		else
		{
			$this->pog_query = "insert into `user` (`firstname`, `lastname`, `email`, `password`, `regdate`, `lastlogindate`, `categoryid` ) values (
