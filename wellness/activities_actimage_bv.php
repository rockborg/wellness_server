...
$activities_actimage_blobview = NULL; // Initialize page object first

class cactivities_actimage_blobview extends cactivities {
	...
	// Page object name
	var $PageObjName = 'activities_actimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('activities', 'x_actimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('actimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('actimage')));
				if (trim(strval($this->Recordset->fields('actimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('actimage') . "\"");
				}
...
// Create page object
if (!isset($activities_actimage_blobview)) $activities_actimage_blobview = new cactivities_actimage_blobview();

// Page init
$activities_actimage_blobview->Page_Init();

// Page main
$activities_actimage_blobview->Page_Main();
?>
<?php
$activities_actimage_blobview->Page_Terminate();
?>
