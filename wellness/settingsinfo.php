...
class csettings extends cTable {
	...
	function __construct() {
		...
		// settingsid
		$this->settingsid = new cField('settings', 'settings', 'x_settingsid', 'settingsid', '`settingsid`', '`settingsid`', 3, -1, FALSE, '`settingsid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->settingsid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['settingsid'] = &$this->settingsid;

		// aboutimage
		$this->aboutimage = new cField('settings', 'settings', 'x_aboutimage', 'aboutimage', '`aboutimage`', '`aboutimage`', 200, -1, TRUE, '`aboutimage`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['aboutimage'] = &$this->aboutimage;

		// abouttext
		$this->abouttext = new cField('settings', 'settings', 'x_abouttext', 'abouttext', '`abouttext`', '`abouttext`', 201, -1, FALSE, '`abouttext`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['abouttext'] = &$this->abouttext;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('settingsid', $rs))
				ew_AddFilter($where, ew_QuotedName('settingsid', $this->DBID) . '=' . ew_QuotedValue($rs['settingsid'], $this->settingsid->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`settingsid` = @settingsid@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->settingsid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@settingsid@", ew_AdjustSql($this->settingsid->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "settingsid:" . ew_VarToJson($this->settingsid->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->settingsid->CurrentValue)) {
			$sUrl .= "settingsid=" . urlencode($this->settingsid->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["settingsid"]) : ew_StripSlashes(@$_GET["settingsid"]); // settingsid
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->settingsid->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->abouttext->EditValue = $this->abouttext->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->settingsid->Exportable) $Doc->ExportCaption($this->settingsid);
					if ($this->aboutimage->Exportable) $Doc->ExportCaption($this->aboutimage);
					if ($this->abouttext->Exportable) $Doc->ExportCaption($this->abouttext);
				} else {
					if ($this->settingsid->Exportable) $Doc->ExportCaption($this->settingsid);
					if ($this->aboutimage->Exportable) $Doc->ExportCaption($this->aboutimage);
				}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->settingsid->Exportable) $Doc->ExportField($this->settingsid);
						if ($this->aboutimage->Exportable) $Doc->ExportField($this->aboutimage);
						if ($this->abouttext->Exportable) $Doc->ExportField($this->abouttext);
					} else {
						if ($this->settingsid->Exportable) $Doc->ExportField($this->settingsid);
						if ($this->aboutimage->Exportable) $Doc->ExportField($this->aboutimage);
					}
