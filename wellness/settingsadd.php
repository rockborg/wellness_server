...
class csettings_add extends csettings {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["settingsid"] != "") {
				...
				$this->setKey("settingsid", $this->settingsid->CurrentValue); // Set up key
			} else {
				$this->setKey("settingsid", ""); // Clear key
	...
	function LoadDefaultValues() {
		$this->aboutimage->Upload->DbValue = NULL;
		$this->aboutimage->OldValue = $this->aboutimage->Upload->DbValue;
		$this->aboutimage->CurrentValue = NULL; // Clear file related field
		$this->abouttext->CurrentValue = NULL;
		$this->abouttext->OldValue = $this->abouttext->CurrentValue;
	}
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("settingsid")) <> "")
			$this->settingsid->CurrentValue = $this->getKey("settingsid"); // settingsid
		else
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->abouttext->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->aboutimage);
	...
	function AddRow($rsold = NULL) {
		...
		if (!$this->aboutimage->Upload->KeepFile) {
			$this->aboutimage->Upload->DbValue = ""; // No need to delete old file
		...
		// abouttext
		$this->abouttext->SetDbValueDef($rsnew, $this->abouttext->CurrentValue, "", FALSE);
		if (!$this->aboutimage->Upload->KeepFile) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->settingsid->setDbValue($conn->Insert_ID());
				$rsnew['settingsid'] = $this->settingsid->DbValue;
				if (!$this->aboutimage->Upload->KeepFile) {
					if (!ew_Empty($this->aboutimage->Upload->Value)) {
