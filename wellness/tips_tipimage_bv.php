...
$tips_tipimage_blobview = NULL; // Initialize page object first

class ctips_tipimage_blobview extends ctips {
	...
	// Page object name
	var $PageObjName = 'tips_tipimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('tips', 'x_tipimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('tipimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('tipimage')));
				if (trim(strval($this->Recordset->fields('tipimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('tipimage') . "\"");
				}
...
// Create page object
if (!isset($tips_tipimage_blobview)) $tips_tipimage_blobview = new ctips_tipimage_blobview();

// Page init
$tips_tipimage_blobview->Page_Init();

// Page main
$tips_tipimage_blobview->Page_Main();
?>
<?php
$tips_tipimage_blobview->Page_Terminate();
?>
