...
class cactivities_edit extends cactivities {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->activitiesid->CurrentValue == "")
	...
	function LoadFormValues() {
		...
		if (!$this->activitiesid->FldIsDetailKey)
			$this->activitiesid->setFormValue($objForm->GetValue("x_activitiesid"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->activitiesid->CurrentValue = $this->activitiesid->FormValue;
	...
	function RenderRow() {
		...
		$this->dateadded->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->actimage);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// acttitle
			$this->acttitle->SetDbValueDef($rsnew, $this->acttitle->CurrentValue, "", $this->acttitle->ReadOnly);

			// actimage
			if (!($this->actimage->ReadOnly) && !$this->actimage->Upload->KeepFile) {
				$this->actimage->Upload->DbValue = $rsold['actimage']; // Get original value
			...
			// actdesc
			$this->actdesc->SetDbValueDef($rsnew, $this->actdesc->CurrentValue, "", $this->actdesc->ReadOnly);

			// actlink
			$this->actlink->SetDbValueDef($rsnew, $this->actlink->CurrentValue, "", $this->actlink->ReadOnly);
...
factivitiesedit.ValidateRequired = true;
<?php } else { ?>
factivitiesedit.ValidateRequired = false; 
<?php } ?>
...
<?php if ($activities->activitiesid->Visible) { // activitiesid ?>
	<div id="r_activitiesid" class="form-group">
		<label id="elh_activities_activitiesid" class="col-sm-2 control-label ewLabel"><?php echo $activities->activitiesid->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $activities->activitiesid->CellAttributes() ?>>
<span id="el_activities_activitiesid">
<span<?php echo $activities->activitiesid->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $activities->activitiesid->EditValue ?></p></span>
</span>
<input type="hidden" data-table="activities" data-field="x_activitiesid" name="x_activitiesid" id="x_activitiesid" value="<?php echo ew_HtmlEncode($activities->activitiesid->CurrentValue) ?>">
<?php echo $activities->activitiesid->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (@$_POST["fa_x_actimage"] == "0") { ?>
<input type="hidden" name="fa_x_actimage" id= "fa_x_actimage" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_actimage" id= "fa_x_actimage" value="1">
<?php } ?>
