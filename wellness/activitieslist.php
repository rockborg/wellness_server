...
class cactivities_list extends cactivities {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->activitiesid->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->activitiesid->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		// Initialize
		$sFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->activitiesid->AdvancedSearch->ToJSON(), ","); // Field activitiesid
		$sFilterList = ew_Concat($sFilterList, $this->acttitle->AdvancedSearch->ToJSON(), ","); // Field acttitle
		$sFilterList = ew_Concat($sFilterList, $this->actimage->AdvancedSearch->ToJSON(), ","); // Field actimage
		$sFilterList = ew_Concat($sFilterList, $this->actdesc->AdvancedSearch->ToJSON(), ","); // Field actdesc
		$sFilterList = ew_Concat($sFilterList, $this->actlink->AdvancedSearch->ToJSON(), ","); // Field actlink
	...
	function RestoreFilterList() {
		...
		// Field activitiesid
		$this->activitiesid->AdvancedSearch->SearchValue = @$filter["x_activitiesid"];
		$this->activitiesid->AdvancedSearch->SearchOperator = @$filter["z_activitiesid"];
		$this->activitiesid->AdvancedSearch->SearchCondition = @$filter["v_activitiesid"];
		$this->activitiesid->AdvancedSearch->SearchValue2 = @$filter["y_activitiesid"];
		$this->activitiesid->AdvancedSearch->SearchOperator2 = @$filter["w_activitiesid"];
		$this->activitiesid->AdvancedSearch->Save();

		// Field acttitle
		$this->acttitle->AdvancedSearch->SearchValue = @$filter["x_acttitle"];
		$this->acttitle->AdvancedSearch->SearchOperator = @$filter["z_acttitle"];
		$this->acttitle->AdvancedSearch->SearchCondition = @$filter["v_acttitle"];
		$this->acttitle->AdvancedSearch->SearchValue2 = @$filter["y_acttitle"];
		$this->acttitle->AdvancedSearch->SearchOperator2 = @$filter["w_acttitle"];
		$this->acttitle->AdvancedSearch->Save();

		// Field actimage
		$this->actimage->AdvancedSearch->SearchValue = @$filter["x_actimage"];
		$this->actimage->AdvancedSearch->SearchOperator = @$filter["z_actimage"];
		$this->actimage->AdvancedSearch->SearchCondition = @$filter["v_actimage"];
		$this->actimage->AdvancedSearch->SearchValue2 = @$filter["y_actimage"];
		$this->actimage->AdvancedSearch->SearchOperator2 = @$filter["w_actimage"];
		$this->actimage->AdvancedSearch->Save();

		// Field actdesc
		$this->actdesc->AdvancedSearch->SearchValue = @$filter["x_actdesc"];
		$this->actdesc->AdvancedSearch->SearchOperator = @$filter["z_actdesc"];
		$this->actdesc->AdvancedSearch->SearchCondition = @$filter["v_actdesc"];
		$this->actdesc->AdvancedSearch->SearchValue2 = @$filter["y_actdesc"];
		$this->actdesc->AdvancedSearch->SearchOperator2 = @$filter["w_actdesc"];
		$this->actdesc->AdvancedSearch->Save();

		// Field actlink
		$this->actlink->AdvancedSearch->SearchValue = @$filter["x_actlink"];
		$this->actlink->AdvancedSearch->SearchOperator = @$filter["z_actlink"];
		$this->actlink->AdvancedSearch->SearchCondition = @$filter["v_actlink"];
		$this->actlink->AdvancedSearch->SearchValue2 = @$filter["y_actlink"];
		$this->actlink->AdvancedSearch->SearchOperator2 = @$filter["w_actlink"];
		$this->actlink->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->acttitle, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->actimage, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->actdesc, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->actlink, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->activitiesid); // activitiesid
			$this->UpdateSort($this->acttitle); // acttitle
			$this->UpdateSort($this->actimage); // actimage
			$this->UpdateSort($this->actlink); // actlink
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				$sOrderBy = "";
				$this->setSessionOrderBy($sOrderBy);
				$this->activitiesid->setSort("");
				$this->acttitle->setSort("");
				$this->actimage->setSort("");
				$this->actlink->setSort("");
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->activitiesid->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"factivitieslistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$this->dateadded->ViewCustomAttributes = "";
			...
			if ($this->actimage->UseColorbox) {
				...
				$this->actimage->LinkAttrs["data-rel"] = "activities_x" . $this->RowCnt . "_actimage";
...
factivitieslist.ValidateRequired = true;
<?php } else { ?>
factivitieslist.ValidateRequired = false; 
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) {
		if ($activities_list->TotalRecs <= 0)
			$activities_list->TotalRecs = $activities->SelectRecordCount();
	} else {
...
<form name="factivitieslistsrch" id="factivitieslistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
...
<?php } ?>
<input type="hidden" name="t" value="activities">
<div id="gmp_activities" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($activities_list->TotalRecs > 0) { ?>
<table id="tbl_activitieslist" class="table ewTable">
<?php echo $activities->TableCustomInnerHtml ?>
<thead><!-- Table header -->
...
<?php if ($activities->activitiesid->Visible) { // activitiesid ?>
	<?php if ($activities->SortUrl($activities->activitiesid) == "") { ?>
		<th data-name="activitiesid"><div id="elh_activities_activitiesid" class="activities_activitiesid"><div class="ewTableHeaderCaption"><?php echo $activities->activitiesid->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="activitiesid"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $activities->SortUrl($activities->activitiesid) ?>',1);"><div id="elh_activities_activitiesid" class="activities_activitiesid">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $activities->activitiesid->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($activities->activitiesid->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($activities->activitiesid->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($activities->acttitle->Visible) { // acttitle ?>
	<?php if ($activities->SortUrl($activities->acttitle) == "") { ?>
		<th data-name="acttitle"><div id="elh_activities_acttitle" class="activities_acttitle"><div class="ewTableHeaderCaption"><?php echo $activities->acttitle->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="acttitle"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $activities->SortUrl($activities->acttitle) ?>',1);"><div id="elh_activities_acttitle" class="activities_acttitle">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $activities->acttitle->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($activities->acttitle->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($activities->acttitle->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($activities->actimage->Visible) { // actimage ?>
	<?php if ($activities->SortUrl($activities->actimage) == "") { ?>
		<th data-name="actimage"><div id="elh_activities_actimage" class="activities_actimage"><div class="ewTableHeaderCaption"><?php echo $activities->actimage->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="actimage"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $activities->SortUrl($activities->actimage) ?>',1);"><div id="elh_activities_actimage" class="activities_actimage">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $activities->actimage->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($activities->actimage->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($activities->actimage->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($activities->actlink->Visible) { // actlink ?>
	<?php if ($activities->SortUrl($activities->actlink) == "") { ?>
		<th data-name="actlink"><div id="elh_activities_actlink" class="activities_actlink"><div class="ewTableHeaderCaption"><?php echo $activities->actlink->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="actlink"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $activities->SortUrl($activities->actlink) ?>',1);"><div id="elh_activities_actlink" class="activities_actlink">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $activities->actlink->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($activities->actlink->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($activities->actlink->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($activities->dateadded->Visible) { // dateadded ?>
	<?php if ($activities->SortUrl($activities->dateadded) == "") { ?>
		<th data-name="dateadded"><div id="elh_activities_dateadded" class="activities_dateadded"><div class="ewTableHeaderCaption"><?php echo $activities->dateadded->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="dateadded"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $activities->SortUrl($activities->dateadded) ?>',1);"><div id="elh_activities_dateadded" class="activities_dateadded">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $activities->dateadded->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($activities->dateadded->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($activities->dateadded->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<span id="el<?php echo $activities_list->RowCnt ?>_activities_activitiesid" class="activities_activitiesid">
<span<?php echo $activities->activitiesid->ViewAttributes() ?>>
<?php echo $activities->activitiesid->ListViewValue() ?></span>
</span>
<a id="<?php echo $activities_list->PageObjName . "_row_" . $activities_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($activities->acttitle->Visible) { // acttitle ?>
		<td data-name="acttitle"<?php echo $activities->acttitle->CellAttributes() ?>>
<span id="el<?php echo $activities_list->RowCnt ?>_activities_acttitle" class="activities_acttitle">
<span<?php echo $activities->acttitle->ViewAttributes() ?>>
<?php echo $activities->acttitle->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($activities->actimage->Visible) { // actimage ?>
		<td data-name="actimage"<?php echo $activities->actimage->CellAttributes() ?>>
<span id="el<?php echo $activities_list->RowCnt ?>_activities_actimage" class="activities_actimage">
<span>
<?php echo ew_GetFileViewTag($activities->actimage, $activities->actimage->ListViewValue()) ?>
</span>
</span>
</td>
	<?php } ?>
...
<span id="el<?php echo $activities_list->RowCnt ?>_activities_actlink" class="activities_actlink">
<span<?php echo $activities->actlink->ViewAttributes() ?>>
<?php echo $activities->actlink->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($activities->dateadded->Visible) { // dateadded ?>
		<td data-name="dateadded"<?php echo $activities->dateadded->CellAttributes() ?>>
<span id="el<?php echo $activities_list->RowCnt ?>_activities_dateadded" class="activities_dateadded">
<span<?php echo $activities->dateadded->ViewAttributes() ?>>
<?php echo $activities->dateadded->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
<?php if (!isset($activities_list->Pager)) $activities_list->Pager = new cPrevNextPager($activities_list->StartRec, $activities_list->DisplayRecs, $activities_list->TotalRecs) ?>
<?php if ($activities_list->Pager->RecordCount > 0) { ?>
<div class="ewPager">
