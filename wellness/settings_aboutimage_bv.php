...
$settings_aboutimage_blobview = NULL; // Initialize page object first

class csettings_aboutimage_blobview extends csettings {
	...
	// Page object name
	var $PageObjName = 'settings_aboutimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('settings', 'x_aboutimage');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 400; // Set default width
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('aboutimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('aboutimage')));
				if (trim(strval($this->Recordset->fields('aboutimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('aboutimage') . "\"");
				}
...
// Create page object
if (!isset($settings_aboutimage_blobview)) $settings_aboutimage_blobview = new csettings_aboutimage_blobview();

// Page init
$settings_aboutimage_blobview->Page_Init();

// Page main
$settings_aboutimage_blobview->Page_Main();
?>
<?php
$settings_aboutimage_blobview->Page_Terminate();
?>
